...
;; live in `gnus-modern-summary-format.el'; navigation and today
;; context commands in `gnus-modern-summary-navigate.el'.
...
(declare-function mail-header-p "nnheader" (header))
...
(declare-function gnus-modern--article-read-summary-keys-advice
                  "gnus-modern-summary-navigate" (function &rest arguments))
...
(defun gnus-modern--summary-insert-thread-lines (renderer threads width)
  "Insert thread title lines for THREADS into the current buffer."
  (let* ((count-width
...
(defun gnus-modern--summary-install ()
  ...
  (unless gnus-modern--summary-enabled
    ...
    (advice-add
     ...
     :around #'gnus-modern--article-read-summary-keys-advice)
...
This is an emergency and debugging command, not a minor mode."
  ...
  (when gnus-modern--summary-enabled
    ...
    (advice-remove
     'gnus-article-read-summary-keys
     #'gnus-modern--article-read-summary-keys-advice)
...
(defun gnus-modern--summary-header ()
  "Return update status and right-aligned Summary statistics."
  (let* ((status (gnus-modern--header-status gnus-modern--update-manager))
         (statistics (gnus-modern--summary-header-statistics))
         (header
          (concat
           status
...
(defun gnus-modern-summary-format-message (header)
  ...
  (let* ((prefix
         ...
         (available
          (max
           0
           (- (gnus-modern--summary-width)
              gnus-modern--summary-prefix-width
              (string-width prefix)
              (string-width date)
              1)))
